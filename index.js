...
const amountPaid = document.querySelector(".amount_paid");
...
const calcBdsCharge = () => {
  ...
  // let numMonthsTillEOM = ppMonths(ppStartDateInput.value, billedUpToInput.value);
  let totalFutureCharges;
  let totalPaymentPlan;

  if (billingCycleInput.value == "unmeasured") {
    // Unmeasured PP
    if (new Date(billedUpToInput.value) <= new Date(bdsRenewal)) {
      totalFutureCharges = calcChargesFromApril(bdsCharge) * uplift;
      charge = totalFutureCharges + Number(charge);
      //CHARGE
      charge > bdsCharge + 60 ? (totalPaymentPlan = bdsCharge + 60) : (totalPaymentPlan = charge);
      totalPaymentPlan -= amountPaid.value;
      monthlyPaymentAmount = totalPaymentPlan / numMonths;
    } else {
    ...
    let chargesFromApril = (bdsCharge / 365) * calcDays("2025-03-31", bdsRenewal) * uplift;
  ...
  <h2>Total payment plan value: £${totalPaymentPlan.toFixed(2)}</h2>
  ${
